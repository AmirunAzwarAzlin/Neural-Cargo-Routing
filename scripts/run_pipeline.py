...
def main():
    ...
    try:
        for i, email in enumerate(emails):
            result = process_email_safe(email, extractor)
        ...
        # However far the run got, always write what we have and close out
        # the run record — a crash partway through must not lose prior work.
        ...
        if supabase_client is not None:
            ...
            finish_pipeline_run(supabase_client, run_id, notes=f"{elapsed:.1f}s for {len(submission)}/{len(emails)} emails")
